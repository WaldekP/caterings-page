export const valuePropositionData = {
  lodz: {
    elasticity: "Oferujemy opcję wyboru dowolnych posiłków z diety. U nas nie ma sztywnego jadłospisu, wszystko dostosujesz pod swoje preferencje. Zdecyduj się te dania dietetyczne, które najbardziej Ci smakują.",
    variety: "Proponujemy urozmaicone menu, zawierające różnorodne, oryginalne i smaczne posiłki. Inspiracji szukamy w kuchniach całego świata, dlatego też przygotowywane przez nas dania nie są nudne.",
    quality: "Stawiamy na najwyższą jakość. Każda dieta, którą posiadamy w ofercie, zostaje przygotowane z najlepszych, świeżych i sprawdzonych składników. Nasze dania dietetyczne gotujemy z zaangażowaniem i pasją.",
  },
  warszawa: {
    elasticity: "Z naszego całego menu, możesz dowolnie wybierać posiłki. Brak sztywnego jadłospisu i opcja dostosowania wszystkiego pod indywidualne preferencje to ogromna zaleta. Samodzielnie zadecyduj, które dania najbardziej Ci zasmakowały, a my zadbamy o całą resztę.",
    variety: "Proponowane przez nas menu jest niezwykle urozmaicone. Szukamy inspiracji w kuchniach z całego świata, stąd tak szeroki wybór. Stawiamy na oryginalność, odpowiedni smak i różnorodność. Od teraz, nie będziesz narzekać na nudę.",
    quality: "Oferujemy najwyższą jakość. Do przygotowania każdego posiłku, używamy wyłącznie najlepszych, świeżych i sprawdzonych produktów. Nasze diety zostały stworzone z pełnym zaangażowaniem oraz pasją.",
  },
  bydgoszcz: {
    elasticity: "AfterFit zawsze dostosowuje się do Klientów. Nasza dieta pudełkowa jest niezwykle elastyczna – Ty sam możesz dowolnie komponować swoje menu. Nie narzucamy sztywnego jadłospisu. Wszystko dopasowujesz pod własne preferencje. Możesz zdecydować się tylko na te dania, które najbardziej Ci smakują.",
    variety: "Oferujemy urozmaicone diety pudełkowe. Nasze menu jest oryginalne, smaczne i przede wszystkim różnorodne. Korzystając z AfterFit nigdy nie będziesz znudzony posiłkami. Inspirujemy się smakami z całego świata, a nasze pomysły wcielamy w codzienne dania.",
    quality: "Dieta pudełkowa to posiłki smaczne i zdrowe. Żeby utrzymać najwyższy poziom, niezbędne są składniki odpowiedniej jakości. Każde danie z oferty AfterFit jest przygotowywane ze świeżych i sprawdzonych składników. Menu układamy z zaangażowaniem oraz pasją, starając się zaspokajać oczekiwania naszych Klientów.",
  },
  krakow: {
    elasticity: "W cateringu dietetycznym AfterFit dowolnie komponujesz swoje menu. Możesz wybierać tylko te dania, które najbardziej Ci zasmakowały. Już teraz dostosuj wszystko pod swoje preferencje.",
    variety: "Dieta pudełkowa z AfterFit to gwarancja różnorodności. Proponujemy oryginalne, smaczne dania, do których inspiracji szukamy w kuchniach całego świata. Nasze jedzenie nie jest nudne! Każdego dnia możesz próbować nowych posiłków!",
    quality: "Odpowiednia jakość to nasz priorytet. Każde danie jest przygotowywane z najlepszych, świeżych składników, pochodzących z pewnych i sprawdzonych źródeł.",
  },
  poznan: {
    elasticity: "Dobieraj dowolne posiłki dietetyczne z menu! U nas, nie ma sztywnego menu. W naszym cateringu dietetycznym możesz zamawiać tylko te dania, które Ci zasmakowały. Ty podejmujesz decyzję, a my troszczymy się o całą resztę.",
    variety: "Nasze posiłki nie są nudne! Jeśli chcesz jeść urozmaicone i oryginalne dania dietetyczne, próbować nowych smaków i czerpać radość z jedzenia, to dobrze trafiłeś! Inspirujemy się kuchnią świata i nie boimy się eksperymentować!",
    quality: "Nasza dieta pudełkowa jest przygotowywana ze świeżych składników, pochodzących ze sprawdzonych źródeł. W cateringu dietetycznym stawiamy na najwyższą jakość!",
  },
  torun: {
    elasticity: "W przeciwieństwie do konkurencji, u nas nie ma sztywnego menu. Opcja ta umożliwia dopasowanie jadłospisu pod własne preferencje. Twoim jedynym zadaniem jest wskazanie potraw, które Ci najbardziej smakują- my zatroszczymy się o całą resztę.",
    variety: "Menu AfterFit jest urozmaicone. Proponujemy oryginalne i smaczne dania, inspirowane kuchniami całego świata. Decydując się na nasze usługi, nie będziesz narzekał, że każdego dnia jesz to samo.",
    quality: "Jakość ma dla nas kluczowe znaczenie. Każde danie, znajdujące się w ofercie AfterFit jest przygotowywane ze zdrowych, świeżych i sprawdzonych składników. Menu układamy z zaangażowaniem i pasją.",
  },
  wroclaw: {
    elasticity: "W AfterFit wybierasz dowolne dania z diety! Skomponuj samodzielnie menu, dostosowując je pod swoje preferencje. Możesz zdecydować się tylko na te posiłki, które najbardziej Ci zasmakowały. U nas nie ma sztywnego jadłospisu!",
    variety: "Czerpiemy inspiracje z kuchni całego świata! Catering dietetyczny AfterFit sprawi, że nie będziesz znudzony powtarzającymi się daniami. Proponujemy Ci urozmaicone menu, w skład którego wchodzą niebanalne, oryginalne i smaczne posiłki!",
    quality: "W AfterFit każde danie jest przygotowywane ze świeżych i sprawdzonych składników! Stawiamy na najwyższą jakość. Menu układamy z pasją i zaangażowaniem, wychodząc naprzeciw oczekiwaniom wszystkich Klientów.",
  },

}
